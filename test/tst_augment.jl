...
@testset "$(str_showcompact(ops))" begin
    ...
    @test typeof(wv) === typeof(invwarpedview(camera,
                                    Augmentor.toaffinemap(NoOp(),rect), Flat()))
    @test wv == camera
    wv1, wv2 = @inferred Augmentor._augment((rgb_rect,camera), ops)
    @test typeof(wv1) === typeof(invwarpedview(rgb_rect,
                                    Augmentor.toaffinemap(NoOp(),rect), Flat()))
    @test typeof(wv2) === typeof(invwarpedview(camera,
                                    Augmentor.toaffinemap(NoOp(),rect), Flat()))
    @test wv1 == rgb_rect
    @test wv2 == camera
end
...
@testset "$(str_showcompact(ops))" begin
    wv = @inferred Augmentor._augment(camera, ops)
    @test typeof(wv) === typeof(invwarpedview(rect,
                                    Augmentor.toaffinemap(NoOp(),rect), Flat()))
...
@testset "$(str_showcompact(ops))" begin
    ...
    @test round.(Float64.(wv), digits=1) ==
            round.(Float64.(rotl90(imresize(rect,2,2))), digits=1)
end
...
@testset "$(str_showcompact(ops))" begin
    ...
    @test typeof(img) <: OffsetArray
    @test round.(Float64.(img), digits=1) ==
            round.(Float64.(rotl90(imresize(rect,2,2))), digits=1)
end
...
@testset "$(str_showcompact(ops))" begin
    ...
    # FIX add restrict_indices(::Base.IdentityUnitRange{UnitRange{Int64}})?
...
ops = Augmentor.ImmutablePipeline(Rotate180(), Crop(5:200,200:500), Rotate90(),
        Crop(50:300, 50:195))
@testset "$(str_showcompact(ops))" begin
    wv = @inferred Augmentor._augment(camera, ops)
    @test typeof(wv) <: SubArray
    ...
    @test @allocated(augment!(out, camera, ops)) <
            @allocated(augment(camera, ops))
end
...
ops = (Rotate(45), CropSize(200,200), Zoom(1.1), ConvertEltype(RGB{Float64}),
        SplitChannels())
@testset "$(str_showcompact(ops))" begin
    ...
    # FIX restrict_indices{::Base.IdentityUnitRange{UnitRange{Int64}}
