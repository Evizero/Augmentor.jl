...
@testset "prepare" begin
    ...
    @test_broken typeof(parent(wv)) <: Interpolations.Extrapolation
...
@testset "$(str_showcompact(ops))" begin
    ...
    @test_broken img == imresize(rect, (4,4))
    v = @inferred Augmentor.unroll_applylazy(ops, view(cameras,:,:,1))
...
@testset "$(str_showcompact(ops))" begin
    ...
    @test_broken typeof(wv) === typeof(view(InvWarpedView(square, Augmentor.toaffinemap(NoOp(),square), fillvalue=Flat()),IdentityRange(1:2),IdentityRange(1:2)))
...
@testset "$(str_showcompact(ops))" begin
    ...
    @test_broken wv2 == rect
end

ops = (Rotate(-90), Crop(1:2,1:3), Rotate90()) # affine forces affine
@testset "$(str_showcompact(ops))" begin
    wv = @inferred Augmentor.unroll_applyaffine(ops, rect)
    @test_broken typeof(wv) === typeof(view(InvWarpedView(rect, Augmentor.toaffinemap(NoOp(),rect), fillvalue=Flat()),IdentityRange(1:2),IdentityRange(1:2)))
    wv2 = @inferred Augmentor.unroll_applylazy(ops, rect)
    @test typeof(wv2) == typeof(wv)
end
...
@testset "$(str_showcompact(ops))" begin
    wv = @inferred Augmentor.unroll_applyaffine(ops, rect)
    @test_broken typeof(wv) === typeof(InvWarpedView(rect, Augmentor.toaffinemap(NoOp(),rect), fillvalue=Flat()))
    @test_broken wv[1:2,1:3] == rot180(rect)
    wv2 = @inferred Augmentor.unroll_applylazy(ops, Augmentor.prepareaffine(rect))
    @test_broken typeof(wv2) === typeof(InvWarpedView(rect, Augmentor.toaffinemap(NoOp(),rect), fillvalue=Flat()))
    @test wv2 == wv
    v = @inferred Augmentor.unroll_applylazy(ops, rect)
    @test v === view(rect, 2:-1:1, 3:-1:1)
    @test v == rot180(rect)
...
@testset "$(str_showcompact(ops))" begin
    wv = @inferred Augmentor.unroll_applyaffine(ops, square)
    @test_broken typeof(wv) === typeof(InvWarpedView(rect, Augmentor.toaffinemap(NoOp(),square), fillvalue=Flat()))
    r1 = InvWarpedView(square, Augmentor.toaffinemap(Rotate180(),square), fillvalue=Flat())
    @test wv == InvWarpedView(r1, Augmentor.toaffinemap(Rotate90(), r1), fillvalue=Flat())
